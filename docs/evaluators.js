...
const a = Object.assign; // shorthand
...
export class EvaluatorLevel {
    constructor({policy, env_class, block_container, level_name, level_info, num_ticks}) {
        a(this, {policy, env_class, block_container, level_name, level_info, num_ticks})
    ...
    start() {
        ...
        let level_promise_success = null;
        let level_promise_reject = null;
        let num_envs = 6;
        let rewards = [];
        ...
        {   // Initialize params via a single trace call.
            const trace_env = new this.env_class("trace_seed");
        ...
        const info_block = block_container
            .append('block')
            .classed('info', true)
        info_block.append('h2').text(this.level_name)
        info_block.append('div').text(this.level_info)
        const navigation = info_block.append('div')
        navigation.append('a')
            .text('Back')
            .attr('href', '#')
            .on('click', () => level_promise_success(-1))
        ...
        return new Promise((success, reject) => {
            level_promise_success = success;
            level_promise_reject = reject;
        }).finally(() => {
            finished = true;
        });
    }
